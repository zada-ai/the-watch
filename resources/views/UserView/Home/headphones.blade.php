...
@section('title', 'Premium Headphones - The Watch')

@section('content')

<div class="bg-gradient-to-r from-blue-600 to-cyan-600 py-6">
    <div class="max-w-7xl mx-auto px-4">
        <span class="text-white font-bold text-sm md:text-base">🎧 Explore Our Complete Headphones Collection 🎧</span>
    </div>
</div>
...
@php
    ...
    if (!isset($products)) {
        $products = HeadphonesProduct::where('active', true)
...
<div class="min-h-screen bg-gray-50">
    <div class="max-w-7xl mx-auto px-4 py-12">
        ...
        <!-- Page Header -->
        <div class="mb-12">
            <h1 class="text-4xl md:text-5xl font-bold text-gray-900 mb-2">Premium Headphones Collection</h1>
            <p class="text-gray-600 text-lg">Experience crystal-clear audio with our premium headphone range</p>
        </div>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
            ...
            @forelse($products as $product)
                @php
                    $image = null;
                    // helper to resolve a stored path into a public URL
                    $resolveImageUrl = function($img) {
                        ...
                        $clean = ltrim($img, '/');
                        // if it's a storage path, check storage/app/public
                    ...
                    // Build a flattened images array for JS use (so data-images is always an array of public URLs)
                    $imagesForJs = [];
                    $imagesForJsUrls = [];
                    if (!empty($product->images) && is_array($product->images)) {
                        ...
                        if (array_is_list($product->images)) {
                            ...
                            // associative per-color -> flatten but normalize slashes
                ...
                <div class="bg-white rounded-xl shadow hover:shadow-2xl hover:-translate-y-2 transition-all duration-300 overflow-hidden">

                    <div class="relative h-64 bg-gray-100 overflow-hidden">
                        <img
                            id="product-img-{{ $product->id }}"
                            src="{{ $src }}"                            loading="lazy"                            data-images='@json($imagesForJsUrls ?? [])'
                            data-index="0"
                    ...
                    <div class="p-4">
                        ...
                        <div class="mt-4 flex gap-2">
                            <a href="{{ route('buy', ['type' => 'headphones', 'id' => $product->id]) }}" class="flex-1 inline-block text-center bg-blue-600 hover:bg-blue-700 text-white py-2 rounded-lg">Buy Now</a>
                            <a href="{{ route('headphones') }}" class="flex-1 inline-block text-center border border-gray-200 text-gray-700 py-2 rounded-lg">View More</a>
                        </div>
                    </div>
                </div>
            ...
            @empty
                  <div class="col-span-full flex flex-col items-center justify-center py-20">
                    ...
                    <p class="text-gray-600 mb-6 max-w-2xl text-center">We're working on bringing a premium Headphones collection to our store. Sign up for updates or explore our other categories meanwhile.</p>
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function updateButtonStyles(activeGender) {
        menBtn.classList.toggle('bg-blue-600', activeGender === 'men');
        ...
        allBtn.classList.toggle('bg-purple-600', activeGender === 'all');
    ...
    menBtn.addEventListener('click', () => {
        window.location.href = '{{ route("headphones") }}?gender=men';
    });

    womenBtn.addEventListener('click', () => {
        window.location.href = '{{ route("headphones") }}?gender=women';
    });

    allBtn.addEventListener('click', () => {
        window.location.href = '{{ route("headphones") }}';
    });
    ...
    document.querySelectorAll('input[type="radio"][data-images]').forEach(radio => {
        radio.addEventListener('change', function () {
            ...
            // Fallback: if we have a flat allImages array and a list of color keys,
            // map color position to an image index so each color shows a different image.
            if (Array.isArray(allImages) && allImages.length && Array.isArray(colorKeys) && colorKeys.length) {
                ...
                // set dataset.images to full array so hover cycles through all images
