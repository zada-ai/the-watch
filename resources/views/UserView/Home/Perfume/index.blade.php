...
@section('title', 'Premium Perfumes - The Watch')

@section('content')

<div class="bg-gradient-to-r from-red-600 to-pink-600 py-6">
    <div class="max-w-7xl mx-auto px-4">
        <span class="text-white font-bold text-sm md:text-base">🌹 Explore Our Premium Perfumes Collection 🌹</span>
    </div>
</div>
...
<div class="min-h-screen bg-gray-50">
    <div class="max-w-7xl mx-auto px-4 py-12">
        ...
        <!-- Page Header -->
        <div class="mb-12">
            <h1 class="text-4xl md:text-5xl font-bold text-gray-900 mb-2">Premium Perfumes Collection</h1>
            <p class="text-gray-600 text-lg">Experience luxury fragrances with our exclusive perfume range</p>
        </div>

        <!-- Filter Buttons -->
        <div class="mb-8 flex gap-4">
            <button id="menBtn"
                class="px-8 py-3 bg-red-600 text-white font-semibold rounded-lg transition-all"
                data-gender="men">
                ♂️ Men
            </button>
            <button id="womenBtn"
...
">
                    ...
                    <div class="p-2 sm:p-3 md:p-4">
                                ...
                                @foreach($product->colors as $colorName => $colorCode)
                                    <label class="cursor-pointer">
                                        <input
                                            ...
                                            data-images='@json($perColor[$colorName] ?? [])'
                                        >
                                        <span
                        ...
                        <div class="mt-4 flex gap-2">
                            <a href="{{ route('buy', ['type' => 'perfume', 'id' => $product->id]) }}" class="flex-1 inline-block text-center bg-red-600 hover:bg-red-700 text-white py-2 rounded-lg">Buy Now</a>
                            <a href="{{ route('perfume') }}" class="flex-1 inline-block text-center border border-gray-200 text-gray-700 py-2 rounded-lg">View More</a>
                        </div>
                    </div>
                </div>
            ...
            @empty
                   <div class="col-span-full flex flex-col items-center justify-center py-20">
                    ...
                    <p class="text-gray-600 mb-6 max-w-2xl text-center">We're working on bringing a premium perfume collection to our store. Sign up for updates or explore our other categories meanwhile.</p>
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function updateButtonStyles(activeGender) {
        menBtn.classList.toggle('bg-red-600', activeGender === 'men');
    ...
    menBtn.addEventListener('click', () => {
        window.location.href = '{{ route("perfume") }}?gender=men';
    });

    womenBtn.addEventListener('click', () => {
        window.location.href = '{{ route("perfume") }}?gender=women';
    });

    allBtn.addEventListener('click', () => {
        window.location.href = '{{ route("perfume") }}';
    });
