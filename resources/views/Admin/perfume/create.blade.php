...
<head>
    ...
    <title>Create Perfume Entry</title>
    ...
    <div class="flex h-screen">
        <div class="w-64 bg-gray-900 text-white p-6 flex flex-col">
            ...
            <nav class="flex-1 space-y-4">
                ...
                <a href="{{ route('admin.perfume.index') }}" class="block px-4 py-2 rounded">🌸 Perfumes</a>
            </nav>
        ...
        <div class="flex-1 overflow-auto">
            <div class="bg-white shadow-md p-6 border-b border-gray-200">
                <h1 class="text-3xl font-bold text-gray-800">Create Perfume Entry</h1>
            </div>
                ...
                <form action="{{ route('admin.perfume.store') }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div class="md:col-span-2">
                            ...
                            <input type="text" name="colors" value='{{ old('colors') }}' placeholder="e.g. floral:#ffd1dc" class="mt-1 block w-full border-gray-300 rounded-md">
                            <p class="text-xs text-gray-500 mt-1">Example: <code>floral:#ffd1dc</code> or <code>{"floral":"#ffd1dc"}</code></p>
                        </div>
                        <div>
                    ...
                    <div class="mt-6">
                        ...
                        <a href="{{ route('admin.perfume.index') }}" class="ml-4 text-gray-600">Cancel</a>
                    </div>
                </form>
            </div>
        </div>
...
<body class="bg-gray-100">
    <div class="flex h-screen">
        <div class="w-64 bg-gray-900 text-white p-6 flex flex-col">
            ...
            <nav class="flex-1 space-y-4">
                <a href="{{ route('admin.dashboard') }}" class="block px-4 py-2 rounded hover:bg-gray-700">📊 Dashboard</a>
                <a href="{{ route('admin.perfume.index') }}" class="block px-4 py-2 rounded">🌸 Perfume</a>
            </nav>
            <form action="{{ route('admin.logout') }}" method="POST">@csrf<button type="submit" class="w-full px-4 py-2 rounded bg-red-600 hover:bg-red-700">🚪 Logout</button></form>
        </div>
        ...
        <div class="flex-1 overflow-auto">
            <div class="bg-white shadow-md p-6 border-b border-gray-200">
                <h1 class="text-3xl font-bold text-gray-800">Add Perfume Entry</h1>
            </div>
                ...
                <form action="{{ route('admin.perfume.store') }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div class="md:col-span-2">
                            ...
                            <input type="file" name="image_files[]" multiple accept="image/*" class="mt-1 block w-full">
                            <input type="text" name="images_text" value='{{ old('images_text') }}' placeholder="optional: path1.jpg, path2.jpg" class="mt-2 block w-full border-gray-300 rounded-md">
                            <p class="text-xs text-gray-500 mt-1">Uploaded files are saved to <strong>storage/perfume</strong>. You can also paste existing paths (comma-separated) or JSON array.</p>
                        </div>
                        <div class="md:col-span-2">
