...
class ToysController extends Controller
{
    ...
    {
        $q = $request->get('q');
        $items = ToysProduct::when($q, function($qry) use ($q){
            $qry->where('name', 'like', "%$q%");
        })->orderBy('id', 'desc')->paginate(20);

        return view('Admin.toys.index', ['items' => $items]);
    }
    ...
    public function store(Request $request)
    {
        ...
        if ($request->hasFile('image_files')) {
            $uploaded = $this->normalizeUploadedPaths($request->file('image_files'), 'toys');
        ...
        return redirect()->route('admin.toys.index')->with('success', 'Toy created');
    }

    public function edit($id)
    {
        $item = ToysProduct::findOrFail($id);
        return view('Admin.toys.edit', ['item' => $item]);
    }
    ...
    public function destroy($id)
    {
        $item = ToysProduct::findOrFail($id);
        $item->delete();
        return redirect()->route('admin.toys.index')->with('success', 'Deleted');
    }
}
